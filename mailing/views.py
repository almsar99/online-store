...
def get_mailing_dashboard_cache_key(user):
    if user_can_view_all_recipients(user) or user_can_view_all_mailings(user):
...
class MailingHomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user_can_view_all_recipients(self.request.user):
            ...
            recipients_queryset = Recipient.objects.filter(owner=self.request.user)
        ...
        if user_can_view_all_mailings(self.request.user):
            ...
            mailings_queryset = Mailing.objects.filter(owner=self.request.user)
            attempts_queryset = Attempt.objects.filter(mailing__owner=self.request.user)

        context["recipients"] = recipients_queryset.order_by("-id")[:5]

        context["messages"] = Message.objects.filter(owner=self.request.user).order_by(
            "-id"
        )[:5]

        context["mailings"] = mailings_queryset.order_by("-id")[:5]

        cache_key = get_mailing_dashboard_cache_key(self.request.user)
        ...
        if cached_statistics is None:
            cached_statistics = {
                ...
                "finished_mailings": mailings_queryset.filter(end_time__lt=now).count(),
            ...
            cache.set(cache_key, cached_statistics, MAILING_DASHBOARD_CACHE_TIMEOUT)
