    ...
    def dispatch(
        ...
        **kwargs
        ...
        if product.status == Product.STATUS_PUBLISHED:
            ...
            messages.error(
                request,
                'Редактировать можно только снятые с публикации товары'
            )
...
class ModerationListView(
    ...
    template_name = (
        'catalog/moderation_products.html'
    )
...
class ProductApproveView(
    ...
    def post(self, request, pk):
        ...
        return redirect(
            'catalog:moderation_products'
        )


class ProductRejectView(
    LoginRequiredMixin,
    View
):
