...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        manage_blog_permission = Permission.objects.get(
            codename="can_manage_blog", content_type=blog_content_type
        )

        view_all_recipients_permission = Permission.objects.get(
            codename="can_view_all_recipients", content_type=recipient_content_type
        )

        view_all_mailings_permission = Permission.objects.get(
            codename="can_view_all_mailings", content_type=mailing_content_type
        )

        disable_mailing_permission = Permission.objects.get(
            codename="can_disable_mailing", content_type=mailing_content_type
        )

        view_all_users_permission = Permission.objects.get(
            codename="can_view_all_users", content_type=user_content_type
        )

        block_user_permission = Permission.objects.get(
            codename="can_block_user", content_type=user_content_type
        )
